...
namespace BH.SDK.Tests.Interop.AfterBeat
{
    ...
    public class AfterBeatEaseMapTests
    {
        [Test]
        ...
        {
            foreach (var name in AfterBeatEaseMap.KnownNames)
            {
                var report = new InteropReport();
                var ease = AfterBeatEaseMap.Import(name, report);
        ...
        {
            var report = new InteropReport();
            var ease = AfterBeatEaseMap.Import("InOutNonsense", report);
        ...
        {
            var report = new InteropReport();
            var ease = AfterBeatEaseMap.Import("OutBounce", report);
        ...
        public void Export_EveryEaseType_ProducesAKnownName()
        {
            ...
            {
                var name = AfterBeatEaseMap.Export(ease);
                CollectionAssert.Contains((System.Collections.ICollection)AfterBeatEaseMap.KnownNames, name);
            }
        }
        ...
        public void RoundTrip_SharedEases_AreStable()
        {
            ...
            foreach (var ease in shared)
                Assert.AreEqual(ease, AfterBeatEaseMap.Import(AfterBeatEaseMap.Export(ease)));
        }
    }

    public class AfterBeatTimeMapTests
    {
        [TestCase(0f, 60, 0)]
        [TestCase(1f, 60, 60)]
        [TestCase(1.5f, 60, 90)]
        ...
        public void ToFrame_ConvertsSeconds(float seconds, int framerate, int expected)
            => Assert.AreEqual(expected, AfterBeatTimeMap.ToFrame(seconds, framerate));
        ...
        public void ToFrame_RoundsToNearest_NotDown()
        {
            ...
            Assert.AreEqual(1, AfterBeatTimeMap.ToFrame(1f / 60f * 0.99f, 60));
        }
        ...
        {
            Assert.AreEqual(FrameRules.MinFrame, AfterBeatTimeMap.ToFrame(-100f, 60));
            Assert.AreEqual(FrameRules.MaxFrame, AfterBeatTimeMap.ToFrame(1e9f, 60));
        }
        ...
        {
            var source = AfterBeatMockData.CreateObject();
            // Move ends at 2s, the object starts at 1s.
            Assert.AreEqual(3f, AfterBeatTimeMap.ResolveEndTime(source), 1e-4f);
        }
        ...
        public void ResolveEndTime_EveryAutokillType_MeansSomethingDifferent()
        {
            ...
            source.AutokillType = (int)AfterBeatAutokillType.LastKeyframeOffset;
            Assert.AreEqual(7f, AfterBeatTimeMap.ResolveEndTime(source), 1e-4f);

            source.AutokillType = (int)AfterBeatAutokillType.FixedTime;
            Assert.AreEqual(5f, AfterBeatTimeMap.ResolveEndTime(source), 1e-4f);

            source.AutokillType = (int)AfterBeatAutokillType.SongTime;
            Assert.AreEqual(4f, AfterBeatTimeMap.ResolveEndTime(source), 1e-4f);
        }
        ...
        public void ResolveSpan_IsNeverShorterThanOneFrame()
        {
            ...
            var span = AfterBeatTimeMap.ResolveSpan(source, 60);
        ...
        public void ExportSpan_UsesFixedTime_SoItDoesNotDependOnKeyframes()
        {
            ...
            AfterBeatTimeMap.ExportSpan(new Models.Primitives.FrameSpan(60, 120), 60, target);

            Assert.AreEqual((int)AfterBeatAutokillType.FixedTime, target.AutokillType);
            Assert.AreEqual(1f, target.StartTime, 1e-4f);
            Assert.AreEqual(2f, target.AutokillOffset, 1e-4f);
        }
    }

    public class AfterBeatRotationTests
    {
        [Test]
        [Author(Metadata.Author.Vertoker)]
        [Category(Metadata.Category.Self)]
        ...
        {
            var accumulated = 0f;
            var first = AfterBeatValueMap.AccumulateRotation(90f, ref accumulated);
            var second = AfterBeatValueMap.AccumulateRotation(90f, ref accumulated);
        ...
        public void Differentiate_IsTheInverseOfAccumulate()
        {
            ...
            for (var i = 0; i < deltas.Length; i++)
                absolute[i] = AfterBeatValueMap.AccumulateRotation(deltas[i], ref accumulated);

            var previous = 0f;
            for (var i = 0; i < deltas.Length; i++)
                Assert.AreEqual(deltas[i],
                    AfterBeatValueMap.DifferentiateRotation(absolute[i], ref previous), 1e-3f);
        }
    }

    public class AfterBeatValueMapTests
    {
        [Test]
        [Author(Metadata.Author.Vertoker)]
        [Category(Metadata.Category.Self)]
        ...
        {
            var key = new VgdKeyframe { RandomType = (int)AfterBeatRandomType.None };
            Assert.IsInstanceOf<FloatValue>(AfterBeatValueMap.ImportFloat(5f, key));
        }
        ...
        public void ImportFloat_LinearRandom_IsARange()
        {
            ...
            {
                RandomType = (int)AfterBeatRandomType.Linear,
                RandomValues = new System.Collections.Generic.List<float> { 3f, 0f, 0f },
            };

            var value = AfterBeatValueMap.ImportFloat(5f, key);
        ...
        public void ImportFloat_Toggle_IsATwoValuedStep()
        {
            var key = new VgdKeyframe
            {
                RandomType = (int)AfterBeatRandomType.Toggle,
                RandomValues = new System.Collections.Generic.List<float> { 4f, 0f, 0f },
            };

            var value = AfterBeatValueMap.ImportFloat(1f, key);
        ...
        public void ImportFloat_Relative_IsDroppedNotSilentlyWrong()
        {
            ...
            var key = new VgdKeyframe
            {
                RandomType = (int)AfterBeatRandomType.Relative,
                RandomValues = new System.Collections.Generic.List<float> { 4f, 0f, 0f },
            };

            Assert.IsInstanceOf<FloatValue>(AfterBeatValueMap.ImportFloat(1f, key, report));
