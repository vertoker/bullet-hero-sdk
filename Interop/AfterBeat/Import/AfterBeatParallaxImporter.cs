...
namespace BH.SDK.Interop.AfterBeat.Import
{
    ...
    public static class AfterBeatParallaxImporter
    {
        /// <summary> How many layers of draw order one parallax layer takes, so objects on
        /// different layers cannot interleave. </summary>
        public const int LayerStride = 10;

        /// <summary> Reads the whole parallax block into the context's scope. </summary>
        public static void ImportAll(VgdParallaxSettings settings, AfterBeatImportContext context,
            int levelFrameDuration, string path)
        {
            ...
            for (var layerIndex = 0; layerIndex < settings.Layers.Count; layerIndex++)
            {
                ...
                var baseLayer = context.Options.ParallaxBaseLayer - layerIndex * LayerStride;
                ...
                for (var i = 0; i < layer.Objects.Count; i++)
                {
                    ...
                    var imported = Import(source, layer, baseLayer, context, levelFrameDuration,
        ...
        private static RectObject Import(VgdParallaxObject source, VgdParallaxLayer layer,
            int baseLayer, AfterBeatImportContext context, int levelFrameDuration, string path)
        {
            var report = context.Report;
            var framerate = context.Options.Framerate;

            var shapeId = AfterBeatShapeMap.Import(source.Shape?.Shape ?? 0, source.Shape?.ShapeOption ?? 0,
            ...
            var color = AfterBeatColorMap.Import(paletteIndex, 1f, AfterBeatPalette.Parallax,
        ...
        private static void Bake(VgdParallaxObject source, ShapeObject target,
            AfterBeatImportContext context, int levelFrameDuration, int framerate, string path)
        {
            ...
            var baseRotation = transform.Rotation * AfterBeatValueMap.DegreesToRadians;
            ...
            var loopFrames = AfterBeatTimeMap.ToFrame(animation.Length, framerate);
            ...
            var loopRotation = animation.Rotation * AfterBeatValueMap.DegreesToRadians;

            var periodFrames = Math.Max(2, loopFrames);
            var startFrame = Math.Max(0, AfterBeatTimeMap.ToFrame(animation.Delay, framerate));
